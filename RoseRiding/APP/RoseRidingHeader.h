//
//  RoseRidingHeader.h
//  RoseRiding
//
//  Created by MR_THT on 2020/3/18.
//  Copyright © 2020 MR_THT. All rights reserved.
//

#ifndef RoseRidingHeader_h
#define RoseRidingHeader_h

//api环境
#define host(path) [NSString stringWithFormat:@"http://rose.leopardtech.co.uk/api/%@", path]
//测试
//#define host(path) [NSString stringWithFormat:@"http://leopard.hbhb.vip/api/%@", path]

#import <SDWebImage/UIImageView+WebCache.h>
#import <YYKit.h>
#import <AFNetworking/AFHTTPSessionManager.h>
//#import <AFNetworking/Reachability.h>
//#import <AFNetworking/NSURLSession.h>
//#import <AFNetworking/NSURLSession.h>
#import <Masonry.h>
#import <MJRefresh/MJRefresh.h>
#import <MJExtension.h>
#import <MBProgressHUD/MBProgressHUD.h>
#import <Toast.h>
#import "NetworkingManger.h"
#import "ConstantData.h"
#import "Languagemanger.h"
#import "GlobalControlManger.h"
#import "UserInfo.h"
#import <ReactiveObjC/ReactiveObjC.h>
#import <IQKeyboardManager/IQKeyboardManager.h>
#import "GYPeripheralInfo.h"
#import "GYBabyBluetoothManager.h"
#import <BabyBluetooth/BabyBluetooth.h>
#import <JXCategoryView.h>
#import "NSObject+MSHUD.h"
#import <GoogleMaps/GoogleMaps.h>
#import <GooglePlaces/GooglePlaces.h>
#import "Toast.h"
#import <JZLocationConverter.h>
#import "GYBabyBluetoothManager.h"
#import <CommonCrypto/CommonDigest.h>
#import "BlueToothAlert.h"
#import "MyDevicemanger.h"
//#import <WMZDialog.h>
#import "WMZDialog.h"
#import "JPUSHService.h"
#import <TwilioVoice/TwilioVoice.h>

#endif /* RoseRidingHeader_h */
